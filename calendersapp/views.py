...
SCOPES = ['https://www.googleapis.com/auth/calendar',
          ...
          'openid']
REDIRECT_URL = 'http://127.0.0.1:8000/calendersapp/v1/calendar/redirect/'
...
def GoogleCalendarInitView(request):
    ...
    # redirect_url = reverse('v1/calendar/redirect')
    # redirect_url += '?' + urlencode({'csrf_token': csrf_token})
    ...
    print("thissssssssssssssssssssss is state in init:"+state)
...
def GoogleCalendarRedirectView(request):
    ...
    state = request.session['state']
    print("thisssssssssssssssssssssssssss is state:"+state)
    flow = google_auth_oauthlib.flow.Flow.from_client_secrets_file(
        CLIENT_SECRETS_FILE, scopes=SCOPES, state=state)
    flow.redirect_uri = REDIRECT_URL
    # Use the authorization server's response to fetch the OAuth 2.0 tokens.
    authorization_response = request.get_full_path()
    print("thisssssssssssssssssss is response:"+authorization_response)
